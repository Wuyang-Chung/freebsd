 ...
 *   /nfs/vm.FreeBSD/sys/net/ifdi_if.m
 * with
 *   makeobjops.awk
 *
...
/** @brief Unique descriptor for the IFDI_QUEUE_INTR_ENABLE() method */
extern struct kobjop_desc ifdi_queue_intr_enable_desc;
/** @brief A function implementing the IFDI_QUEUE_INTR_ENABLE() method */
typedef int ifdi_queue_intr_enable_t(if_ctx_t _ctx, uint16_t _qid);

static __inline int IFDI_QUEUE_INTR_ENABLE(if_ctx_t _ctx, uint16_t _qid)
{
	kobjop_t _m;
	KOBJOPLOOKUP(((kobj_t)_ctx)->ops,ifdi_queue_intr_enable);
	return ((ifdi_queue_intr_enable_t *) _m)(_ctx, _qid);
}
...
typedef void ifdi_crcstrip_set_t(if_ctx_t _ctx, int _onoff);

static __inline void IFDI_CRCSTRIP_SET(if_ctx_t _ctx, int _onoff)
{
	kobjop_t _m;
	KOBJOPLOOKUP(((kobj_t)_ctx)->ops,ifdi_crcstrip_set);
	((ifdi_crcstrip_set_t *) _m)(_ctx, _onoff);
}
